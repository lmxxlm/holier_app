...
public class DoctorController {
  ...
  @RequestMapping(value = ViewName.doctor_manage)
  public ModelAndView doctor_manage(HttpServletRequest request, HttpServletResponse response) {
    ...
    view.addObject(ViewName.menuIndex, ViewName.doctor_manage);
    view.setViewName(ViewName.doctor_manage);
    view.addObject(JSTLField.mainUrl, mainUrl.trim());
    // 列表页数据
    KfData kfdata = new KfData(request);
    kfdata.put(Config.cake_uri, StringUtils.join(new String[] { cake_uri, Config.cake_doctor_list }));
  ...
  @RequestMapping(value = ViewName.doctor_manage_add)
  @ResponseBody
  public boolean doctor_manage_add(HttpServletRequest request, HttpServletResponse response) {
    ...
    KfData kfdata = new KfData(request);
    kfdata.put(Config.cake_uri, StringUtils.join(new String[] { cake_uri, Config.cake_doctor }));
    return commEntityModule.add(kfdata);
  }

  @RequestMapping(value = ViewName.doctor_manage_edit)
  @ResponseBody
  public boolean doctor_manage_edit(HttpServletRequest request, HttpServletResponse response) {
    // medex每个post请求都需验证token,，如果验证不成功，直接返回失败，否则，token置空(防止重复提交)，并进行下一步
    if (tokenUtil.matchToken(request) == TokenUtil.code_mismatch) { return false; }
  ...
  @RequestMapping(value = ViewName.doctor_manage_delete)
  @ResponseBody
  public boolean doctor_manage_delete(HttpServletRequest request, HttpServletResponse response) {
    // medex每个post请求都需验证token,，如果验证不成功，直接返回失败，否则，token置空(防止重复提交)，并进行下一步
    if (tokenUtil.matchToken(request) == TokenUtil.code_mismatch) { return false; }
  ...
  @RequestMapping(value = ViewName.find_doctor, produces = "application/string; charset=utf-8")
  @ResponseBody
  public String find_doctor(HttpServletRequest request, HttpServletResponse response) {
    Map<String, Object> result = new HashMap<>();
    KfData kfdata = new KfData(request);
    kfdata.put(Config.cake_uri, StringUtils.join(new String[] { cake_uri, Config.cake_doctor_find }));
