...
<body>
	...
	<div class="container-fluid">
		...
		<div id="content">
			...
			<ol class="breadcrumb webmap">
				...
				<li class="active">病区管理</li>
			</ol>
			
			<div class="searchBar row">
				<div class="smartSearch col-md-3 col-xs-3">
				    <form  id="searchForm" action="${mainUrl}illness_area_manage.do" method="post">
				     <input type="hidden" name="cp" value="">
				     <input type="hidden" name="ps" value="${searchParam.ps}">
				     <input type="hidden" name="token" value="${token}">
					<div class="input-group">
						<input type="hidden" name="illAreaId" value="${illAreaId}"> <input
							type="text" class="form-control" name="illAreaName"
							placeholder="请输入病区名称" autocomplete="off" value="${illAreaName}">
			...
			<div class="mainBody">
				...
				<table class="table table-striped table-hover">
					...
					<tbody>
						<c:if test="${noproduct == true}">
							<tr>
								<td colspan="9">请添加病区管理数据。</td>
							</tr> 
						</c:if>
						<c:forEach items="${data.records}" var="area">
							<tr>
								<td>${area.hospital}</td>
								<td>${area.name}</td>
								<td>${area.treat_num}</td>
								<td>${area.remark}</td>
								<td class="td1">
									<a href="javascript:void(0);" data-id="${area._id}" class="edit_manage">
									<span class="glyphicon glyphicon-edit"></span>修改</a>
									<a href="javascript:void(0);" data-id="${area._id}" class="delete_manage">
				...
				<div align="center">
					<ul class="pagination">
					   ...
					   <!-- <a href="${mainUril}illness_area_manage.html?cp=${iam.pageView.back}&&ps=${searchParam.ps}">上一页</a> --> 
					   <a href="javascript:void(0);" class="divPage"  data-name="back">上一页</a> 
						</li>
						<li <c:if test="${iam.pageView.last == searchParam.cp}"> class="disabled"</c:if>>
					 <!--   <a href="${mainUril}illness_area_manage.html?cp=${iam.pageView.next}&&ps=${searchParam.ps}">下一页</a>--> 
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="adminModalLabel">病区管理</h4>
				</div>
				<div class="modal-body">
					...
					<div class="form-group">
						<label for="illAreaName">病区名称</label> 
							<input type="text" class="form-control" name="illAreaName" 
								placeholder="请输入病区名称" autocomplete="off">
				...
				<div class="modal-footer">
					<input type="hidden" name="token" value="${token}"> 
					<input type="hidden" name="illAreaId" value="">
	...
	$(function(){
		...
		function initModal(){
			...
			$('#adminModal input[name="illAreaId"]').val('');
		...
		function doAdd(){
			...
			var questURL = runParams.mainUrl+'illness_area_manage_add.do';
			var questArr = {
				hospital:hospital,
				name:illAreaName,
				treat_num:docNum,
			...
			$.post(questURL,questArr,function(result){
				if(result){
					location.href=runParams.mainUrl+"illness_area_manage.html";
				}else{
					alert('添加失败！');
					return false;
				}
		...
		$('.edit_manage').click(function(){
			...
			var questURL=runParams.mainUrl+'find_illness_area.do';
			var questArr={comm_recordId:id}
			$.post(questURL,questArr,function(result){
				result = eval("("+result+")");   
				if(1==result.status){
					$('#adminModal select[name="hospital"]').val(result.data.hospital);
					$('#adminModal input[name="illAreaName"]').val(result.data.name);
					$('#adminModal input[name="docNum"]').val(result.data.treat_num);
					$('#adminModal input[name="remark"]').val(result.data.remark);
					$('#adminModal input[name="token"]').val(result.data.token);
			...
			$('#adminModal input[name="illAreaId"]').val(id);
			$('#adminModal input[name="actionType"]').val('add');
			$('#adminModal').modal();
		});
		...
		function doEdit(){
			...
			var illAreaId = $('#adminModal input[name="illAreaId"]').val();
			var hospital=$('#adminModal select[name="hospital"]').val();
			var illAreaName=$('#adminModal input[name="illAreaName"]').val();
			var docNum=$('#adminModal input[name="docNum"]').val();
			var remark=$('#adminModal input[name="remark"]').val();
			...
			var questURL = runParams.mainUrl+'illness_area_manage_edit.do';
			var questArr = {
					_id:illAreaId,
					hospital:hospital,
					name:illAreaName,
		...
		$(".delete_manage").click(function(){
		    ...
		    if(confirm('确定要把分类“'+name+'"从列表中删除吗？')){
				//查找该分类下是否已经有产品
				var questURL = runParams.mainUrl+'illness_area_manage_delete.do';
				var questArr = {
						comm_recordId:id,
						token:token
						};
