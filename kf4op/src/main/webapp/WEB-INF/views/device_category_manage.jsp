...
<body>
			...
			<div class="searchBar row">
				<div class="smartSearch col-md-3 col-xs-3">
					...
					<div class="input-group">
						<input type="hidden" name="deviceId" value=""> <input
							type="text" class="form-control" name="categoryDeviceName"
							placeholder="请输入设备类别名称" autocomplete="off" value="">
			...
			<div class="mainBody">
				...
				<table class="table table-striped table-hover">
					...
					<tbody>
						...
						<c:forEach items="${data.records}" var="category">
							<tr>
								<td>${category.name}</td>
								<td>${category.hospital}</td>
								<td>${category.icon_info}</td>
								<td>${category.remark}</td>
								<td class="td1">
									<a href="javascript:void(0);" data-id="${category._id}" class="edit_manage">
									<span class="glyphicon glyphicon-edit"></span>修改</a>
									<a href="javascript:void(0);" data-id="${category._id}" class="delete_manage">
				...
				<div align="center">
					<ul class="pagination">
						<li <c:if test="${searchParam.cp == 1}"> class="disabled"</c:if>>
					        <!--  <a href="${mainUril}category_manage.html?cp=${dev.pageView.back}&&ps=${searchParam.ps}">上一页</a>-->
						...
						<li     
					      ...
					      <!--   <a href="${mainUril}category_manage.html?cp=${dev.pageView.next}&&ps=${searchParam.ps}">下一页</a> --> 
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="form-group">
						<label for="categoryDeviceName">设备类别名称</label>
						<input type="text" class="form-control" name="categoryDeviceName" 
	...
	$(function(){
		...
		function initModal(){
			...
			$('#adminModal input[name="categoryDeviceName"]').val('');
		...
		//添加逻辑
		function doAdd(){
			var categoryDeviceName=$('#adminModal input[name="categoryDeviceName"]').val();
			...
			var questURL = runParams.mainUrl+'device_category_manage_add.do';
			var questArr = {
				name:categoryDeviceName,
				hospital:hospital,
				icon_info:iconInfo,
				remark:remark,
				token:token
			...
			$.post(questURL,questArr,function(result){
				if(result){
					location.href=runParams.mainUrl+"device_category_manage.html";
				}else{   
					alert('添加失败！');
					return false;
				}
		...
		$('.edit_manage').click(function(){
			...
			var questURL=runParams.mainUrl+"find_device_category.do";
			var questArr={comm_recordId:id};
			$.post(questURL,questArr,function(result){
				result = eval("("+result+")");   
				if(1==result.status){
					
					$('#adminModal input[name="categoryDeviceName"]').val(result.data.name);
					$('#adminModal select[name="hospital"]').val(result.data.hospital);
					$('#adminModal input[name="iconInfo"]').val(result.data.icon_info);
		...
		function doEdit(){
			...
			var questURL = runParams.mainUrl+'device_category_manage_edit.do';
			var questArr = {
					name:categoryDeviceName,
					_id:deviceId,
					hospital:hospital,
		...
		$(".delete_manage").click(function(){
		    ...
		    if(confirm('确定要把分类“'+name+'"从列表中删除吗？')){
				//查找该分类下是否已经有产品
				var questURL = runParams.mainUrl+'device_category_manage_delete.do';
				var questArr = {
						comm_recordId:id,
						token:token
						};
