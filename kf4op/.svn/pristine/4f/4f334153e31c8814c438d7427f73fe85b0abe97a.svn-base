...
public class DepartmentController {
  ...
  @RequestMapping(value = ViewName.departments_manage)
  public ModelAndView departments_manage(HttpServletRequest request, HttpServletResponse response) {
    ...
    view.addObject(ViewName.menuIndex, ViewName.departments_manage);
    view.setViewName(ViewName.departments_manage);
  ...
  @RequestMapping(value = ViewName.departments_manage_add)
  @ResponseBody
  public boolean departments_manage_add(HttpServletRequest request, HttpServletResponse response) {
    ...
    KfData kfdata = new KfData(request);
    kfdata.put(Config.cake_uri, StringUtils.join(new String[] { cake_uri, Config.cake_department }));
    return commEntityModule.add(kfdata);
  }

  @RequestMapping(value = ViewName.departments_manage_edit)
  @ResponseBody
  ...
  @RequestMapping(value = ViewName.departments_manage_delete)
  @ResponseBody
  ...
  @RequestMapping(value = ViewName.find_department, produces = "application/string; charset=utf-8")
  @ResponseBody
  public String find_department(HttpServletRequest request, HttpServletResponse response) {
    Map<String, Object> result = new HashMap<>();
    KfData kfdata = new KfData(request);
    kfdata.put(Config.cake_uri, StringUtils.join(new String[] { cake_uri, Config.cake_department_find }));
