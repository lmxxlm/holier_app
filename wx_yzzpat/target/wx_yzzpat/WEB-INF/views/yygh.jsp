...
.bg .modal div.info .title i.close {
	...
	width: 30px;
	height: 40px;
	background: url(assets/image/experts_details/close.png) no-repeat 50%
		50%;
...
.bg .modal div.info .img img { /*width:23%;*/
	max-width: 170px;
	width: 85px;
	height: 85px;
	border-radius: 50%;
...
<body>
		...
		<div class="wrap scroller">
			...
			<ul class="lists scroller">
				...
				<c:forEach items="${doctors}" var="doctor">
					<li><a href="#" class="scroller"> <input type="hidden"
							name="showVal" data_accountid="${doctor.accountid}" data_name="${doctor.name}"
							...
							data_doctorIntroduce="${doctor.remark}" /> <img class="docImg"
							<c:choose><c:when test="${not empty doctor.headPic.middle}">src="${doctor.headPic.middle}"</c:when><c:otherwise>src="assets/image/expert_list/head.png"</c:otherwise></c:choose>>
							<span> <b class="name">${doctor.name}</b> <b>${doctor.doctorTitle}</b>
								<b>${doctor.referDepartments}</b> <b>${doctor.referHospital}</b>
						</span> <!--  <i class="collection"><img src="assets/image/expert_list/default_collection.png"></i> -->
	...
	<div class="bg">
		<div class="modal">
			<div class="info">
				<div class="title">
					<span>专家详情</span><i class="close"></i>
				</div>
				<div class="img">
					<img src="assets/image/experts_details/defult_head.png">
					<a href="#" class="yygh">点击挂号</a>
				</div>
				<div class="intro">
					<p>
						<b class="name">将攻打</b><b class="doctorTitle">副主任医师</b>
					</p>
			...
			<div class="introduce">
				...
				<p class="p2 doctorIntroduce">
					dhdjkjlfdskjlfdskldsfk，觉得手机开放的健康的是否健康的是否健康开放的数据库开发的艰
					苦奋斗空间的首付款健康的说法加快分解开放的健康的是否健康考了多少分艰苦奋斗时间肯定是妇科健康的
					看课件发到手机开放的设计的设计的数据库看肌肤抵抗力降低首付款决定是否健康的说法开朗大方款式的罚款决定书
					疯狂的疯狂就是对方可能是对方考虑的是快乐简单快乐的克里夫抵抗力降低首付款了解对方考虑加分来看风口浪尖的付款即
					可京东方科技的方式空间发动机肯定是凤姐开放的时间都是风景进口发动机考古发掘开放的健康的酒店房间靠进口发动机发动
					...
					疯狂的疯狂就是对方可能是对方考虑的是快乐简单快乐的克里夫抵抗力降低首付款了解对方考虑加分来看风口浪尖的付款即
					可京东方科技的方式空间发动机肯定是凤姐开放的时间都是风景进口发动机考古发掘开放的健康的酒店房间靠进口发动机发动
					机客服电话酒店房间开发空间的疯狂觉得十分艰苦地方即可打发时间空间打开的风景浪费大家看看对方健康减肥快点放开经历过的快乐</p>
			</div>
	...
	<script>
		...
		$(function() {
			...
			function init() {
				...
				$('.modal').find('.img a').attr('href', '');
			}
			function center(obj) {
				//obj这个参数是弹出框的整个对象  
				var screenWidth = $(window).width(), screenHeigth = $(window)
						.height();
				//获取屏幕宽高  
			...
			$('li a').live(
					...
					function() {
						init();
						//获取值
						var accountid = $(this).find('input[name="showVal"]').attr(
								'data_accountid');
						var name = $(this).find('input[name="showVal"]').attr(
								'data_name');
						var doctorTitle = $(this).find('input[name="showVal"]')
								.attr('data_doctorTitle');
						var referDepartments = $(this).find(
								'input[name="showVal"]').attr(
								'data_referDepartments');
						var referHospital = $(this).find(
								'input[name="showVal"]').attr(
								'data_referHospital');
						var doctorSpecial = $(this).find(
								'input[name="showVal"]').attr(
								'data_doctorSpecial');
						var doctorIntroduce = $(this).find(
								'input[name="showVal"]').attr(
								'data_doctorIntroduce');
						...
						$('.modal').find('.img a').attr('href', 'yspb?account_id='+accountid);
