...
<html class="no-js" lang=”zh”>
	...
	<body>
		...
		<div class="main">
			...
			<div class="head">
				<input type="hidden" name="selectArea" value="" data-flag=""/>
				<form action="search_kf_hospital.do" method="post">
	...
	<script type="text/javascript">
			...
			function init() {
				...
				// 百度地图API功能
				var map = new BMap.Map("allmap",{minZoom:4,maxZoom:16});
				...
				var flag=$('input[name="selectArea"]').attr('data-flag');
				if(selectName){
					...
					function searchByStationName() {
						...
						　　　　var poi = searchResult.getPoi(0);
						     ...
						     var markerSearch=addLabel(searchPoint,selectName,10);
						　　　　map.centerAndZoom(poi.point, 10);
						     markerSearch.addEventListener("click",function(e){
								 var p = e.target;
					            getPoints(flag,selectName,p);
				             })
				...
				var hz = new BMap.Point(120.22,30.3);
				var sh =new BMap.Point(121.49,31.3);
				var markerHZ=addLabel(hz,'浙江省',6);
				var markerSH=addLabel(sh,'上海市',6);
				markerHZ.rangeLev={min:4,max:7};
				markerSH.rangeLev={min:4,max:7};
				
				markerHZ.addEventListener("click",function(e){
					 var p = e.target;
					 var flag={province:'浙江省',city:'',area:''};
		            getPoints(flag,p);
				})
				
				markerSH.addEventListener("click",function(e){
					 var p = e.target;
					 var flag={province:'上海市',city:'',area:''};
		             getPoints(flag,p);
				})
				
				function getPoints(flag,name,p){
					var questArr={
						flag:name
					};
					...
					*/
					//造假数据(也就是从后台获取的数据)
					    ...
					    cbs.push({count:10,name:"杭州市",flag:"city",children:[{count:2,lat:30.0,lng:120.0,name:"下城区",flag:"area"}],prename:"浙江省",preflag:"province"});
		                cbs.push({region_name:"西湖区",lat:30.2643510000,lng:120.1351770000,hos_count:3,flag:{province:'浙江省',city:'杭州市',area:'西湖区'}});
						cbs.push({region_name:"上城区",lat:30.2468820000,lng:120.1754210000,hos_count:2,flag:{province:'浙江省',city:'杭州市',area:'上城区'}});
						cbs.push({region_name:"下城区",lat:30.2878050000,lng:120.1903690000,hos_count:1,flag:{province:'浙江省',city:'杭州市',area:'下城区'}});
						cbs.push({region_name:"江干区",lat:30.2613570000,lng:120.2173900000,hos_count:4,flag:{province:'浙江省',city:'杭州市',area:'江干区'}});
						cbs.push({region_name:"拱墅区",lat:30.3217290000,lng:120.1501250000,hos_count:2,flag:{province:'浙江省',city:'杭州市',area:'拱墅区'}});
						cbs.push({region_name:"滨江区",lat:30.2109350000,lng:120.2168150000,hos_count:5,flag:{province:'浙江省',city:'杭州市',area:'滨江区'}});
						cbs.push({region_name:"萧山区",lat:30.1849640000,lng:120.2714320000,hos_count:3,flag:{province:'浙江省',city:'杭州市',area:'萧山区'}});
						//在加之前删除以前的数据或者隐藏
						//deletePoint();
						//放大地图
						var times;
						flag=='province'?times=2:times=4;
						rollUp(p , times);
						
						for(var i = 0 ; i< cbs.length ; i++){
						    var content= '<p>'+cbs[i].region_name +'</p><p>'+ cbs[i].hos_count +'家</p>';
							var marker = addLabel(cbs[i],content , map.getZoom());
							marker.name=cbs[i].region_name;
							marker.flag=cbs[i].flag;
							//添加范围
							if(cbs[i].flag=='province'){
								marker.rangeLev={min:4,max:7};
							}else if(cbs[i].flag=='city'){
								marker.rangeLev={min:7,max:10};
							}else if(cbs[i].flag=='area'){
								marker.rangeLev={min:10,max:13};
							}else{
								marker.rangeLev={min:13,max:16};
							}
				...
				*/
			    ...
			    //缩放结束后  TODO
			     map.addEventListener("zoomend",function(){
					...
					for (var i = 1; i < allOverlay.length ; ++i){
						...
						if(zoomend > range.min && zoomend < range.max){
							allOverlay[i].show();
						}else{
							allOverlay[i].hide();
						}
			    ...
			    function showJH(){
					...
					var allOverlay = map.getOverlays();
					for (var i = 0; i < allOverlay.length ; i++){
						markers.push(new BMap.Marker(allOverlay[i].point));
						//allOverlay[i].hide();
		...
		<script>
        	...
        	*/
			...
			function getHosNum() {
				...
				$(district){
					$('input[name="selectArea"]').val(district);
					$('input[name="selectArea"]').attr('data-flag','area');
				}else{
					$('input[name="selectArea"]').val(city);
					$('input[name="selectArea"]').attr('data-flag','city');
				}
