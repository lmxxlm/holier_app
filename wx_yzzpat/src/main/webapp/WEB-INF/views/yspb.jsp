...
#radio   ~label { /* background-color: grey;*/
...
<body>
	<div class="main">
		...
		<div class="notice">
			<input type="hidden" name="doctor_id" value="${doctor.accountid}">
			<input type="hidden" name="doctor_name" value="${doctor.name}">
			<p class="p1">${doctor.name}医生 的坐诊时间：</p>
			<hr>
			<br>
			<c:forEach items="${doctor.zz}" var="zz">
				<input name="pbtime" type="radio" style="width: 15px; height: 15px"
					value="${zz.key} -- ${zz.value}">  ${zz.key} -- ${zz.value}
					<br>
				<br>
				<br>
			</c:forEach>
		...
		<div class="writeInfo">
			<p class="patientName">
				<span>姓名</span><input name="patientName" type="text"
	...
	<script>
		$('#confirm').on('click', function() {
			var doctor_id = $('.notice input[name="doctor_id"]').val();
			var doctor_name = $('.notice input[name="doctor_name"]').val();
			var time = $('.notice input[name="pbtime"]:checked').val();
			...
			$.post(questURL, questArr, function(result) {
				if (result) {
					...
					window.location.href = "yygh.html";
				} else {
					alert("系统开小差了，请稍后重试。");
					return false;
				}
