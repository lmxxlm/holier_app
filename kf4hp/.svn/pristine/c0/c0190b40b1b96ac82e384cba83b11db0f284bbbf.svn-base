...
<body>
    <div id="frameMain">
        <div id="top_nav">当前位置：<a href="javascript:;">订单管理</a>&nbsp;>&nbsp;已驳回申请</div>
...
$(function(){
	...
	function menuInit(x,y){
		...
		leftFrame.find('.reject').addClass("active");
...
$(function(){
	...
	function common_page(page){
		$("#submitForm").attr("action", "reject.html?page="+page+"&&hospital_id="+hospital_id).submit();
	}
    //跳页
	...
	function jumpInputPage(totalPage){
		...
		if($('#jumpNumTxt').val()!=''){
			...
			$('#submitForm').attr('action','reject.html?page='+pageNum+"&&hospital_id="+hospital_id).submit();
		}else{
			//跳转页数为空
			alert("您当前没有输入页数");
			$('#submitForm').attr('action','reject.html?page='+1+"&&hospital_id="+hospital_id).submit();
		}
	}
