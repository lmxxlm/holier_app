...
import com.danrong.wx.qzfb.util.comm.JsonUtil;
...
public class OAuthUtil {
  ...
  private OAuthAccessToken generateOAuthAccessToken(String code) {
    CResult cr = spider.crawl(SpiderFlag.WX, wx_suffix_oauth_access_token, Config.charset, code);
  ...
  private SnsUserInfo generateSnsUserInfo(String oauthAccessToken, String openid) {
    CResult cr = spider.crawl(SpiderFlag.WX, wx_get_sns_userinfo, Config.charset, oauthAccessToken, openid);
    if (cr != null && !cr.isEmpty()) {
      String content = cr.content;
